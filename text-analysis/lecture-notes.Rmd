...
3. Distinctive word stuff came from Justin Grimmer's class on [Text as Data](https://www.stanford.edu/~jgrimmer/Text14/)
...
```{r}
docs <- Corpus(DirSource("Data/MachText/.csv")) 
docs
```
...
```{r}
docs.df <-read.csv("Data/shelbysessions.csv", header=TRUE) #read in CSV file
...
```{r}
# Read corpus from csv - you already did this in Step 1.1
docs.df <- read.csv("Data/shelbysessions.csv")
```
...
findFreqTerms(dtm, lowfreq=1000) # words that appear at least 100 times

# Which words correlate with "war"?
findAssocs(dtm, "war", 0.4)
...
dtm$doc_author <- docs.df$author

# check to see if they're the same number of documents per author
summary(dtm$doc_author)
summary(docs.df$author)
```

## 2. Discriminating Word Analysis
...
```{r}
# Subset into 2 dtms
shelby <- subset(dtm,doc_author=="Shelby",select = -doc_author) # should have 1102 obs.
sessions <- subset(dtm,doc_author=="Sessions",select = -doc_author) # should have 236 obs.
```
...
## 3. Sentiment Analysis and Quantitative Analysis of Text
...
## 3.1 Comparing Songs on the Thriller Album
...
## 3.2 Visualizing Plot using Sentiments
...
Mallet a popular topic modelling software application. It is in java - which means it's fast - but R has a Mallet wrapper package.
