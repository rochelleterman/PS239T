...
Because matrices are implemented as vectors with special attributes, you can subset them with a single vector. In that case, they will behave like a vector, with one column tacked onto another:

```{r}
(vals <- matrix(letters[1:10], nrow = 5, ncol = 2))
vals[c(1, 5, 6)]
```
...
Check your answers in [answers](#subsetting-answers).
