...
* [Data types](#data-types) starts by teaching you about `[`. You'll start
  by learning the four ways to subset atomic vectors. You'll then learn how 
  those four methods act when used to subset lists, matrices, and data frames.
...
* [Applications](#applications) leads you through important, but
  not obvious, applications of subsetting to solve problems that you
  often encounter in a data analysis, using the tools above.
...
#### 3.   __Logical vectors__ select elements where the corresponding logical 
    value is `TRUE`.
...
mtcars[ , 2:11] # like a matrix, all rows, columns 2 through 11
mtcars[2:11] # like a list, taking all columsn 2 through 11

#using negative integers
mtcars[ , -1] # like a matrix, all rows, all columns except first column
mtcars[-1] # like a list, all columns except first

#using logical
mtcars[, !colnames(mtcars) %in% c("mpg")] # like a matrix, all rows, all columns except columns called "mpg"
mtcars[!colnames(mtcars) %in% c("mpg")]  # like a list, all columns except columns called "mpg"

#using names
mtcars[, c("cyl","disp","hp","drat","wt", "qsec","vs","am","gear","carb")]
mtcars[c("cyl","disp","hp","drat","wt", "qsec","vs","am","gear","carb")]

# by deleting columns explicitely
mtcars$mpg <- NULL # delete column "mpg"

and since mtcars$mpg == mtcars[["mpg"]], we can also use
