...
In R, the contents of an object can be printed by either simply executing the the object name or calling the ```print()``` function.  Note that, unlike in python, ```print()``` must have object names passed directly to it as arguments inside parentheses.
...
gsub(pattern = " ", replacement = "", x = fullName) # same function used for replacements and stripping
```

### Compound Data Types: Vectors, Matrices, Arrays, Data Frames, and Lists
...
4.  ***Data Frames*** are two-dimensional and contain objects drawn from any number of classes, including integer, numeric, character, and factor.
5.  ***Lists*** are one-dimensional and contain objects drawn from any class, including lists themselves.

All of these object types are ordered and indexable, as with python objects.  While indexing methods are fairly constant across object types, there are a few differences.  For the simplest object type --- vectors --- indexing works just as with python objects.
...
To add items to the end of a vector, or to combine multiple vectors, you can use the ```c()``` function.  Note that, unlike with python's append or extend methods, you must assign the result of any ```c()``` call to an object.  Often, it is convenient to simply overwrite a pre-existing object.

```{r}
country.vector<-c(country.vector, "Brazil")
country.vector<-c(country.vector, c("Vietnam", "South Africa"))
print(country.vector)
```
...
print(country.matrix)
class(country.matrix)
is(country.matrix)
is.character(country.matrix)
country.matrix[4,2]
country.matrix[2:4, 1]
country.matrix[1:3,]
country.matrix[,2]
country.matrix[-1,]
country.matrix[-c(1,3,5),]
```
