################################################
#### Prepared for PS236B section only
#### Code written by Shinhye Choi
#### contact: schoi@berkeley.edu for suggestions
...
################################################
setwd("~/Dropbox/berkeley/Dissertation/Data and Analyais/Git Repos/PS239T/12_GeoSpatial/data")
...
setwd("~/Downloads/Oakland_blkgrp")
...
plot(sp_dat[over(buff, sp_dat, returnList = TRUE)[[1]],], add=T, col="red")
plot(buff, col="yellow", add=T)

# find the neighborhoods
study <- sp_dat[over(buff, sp_dat, returnList = TRUE)[[1]],]
